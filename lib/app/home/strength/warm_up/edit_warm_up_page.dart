...
class EditWarmUpPage extends StatefulWidget {
  const EditWarmUpPage({Key key, this.training, this.trainingApi})
      : super(key: key);
  ...
  static Future<void> show(BuildContext context, {Training training}) async {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => EditWarmUpPage(
  ...
  @override
  _EditWarmUpPageState createState() => _EditWarmUpPageState();
}

class _EditWarmUpPageState extends State<EditWarmUpPage> {
  ...
  Future<void> _submit() async {
    if (_validateAndSaveForm()) {
      try {
        final warmUpApi = await widget.trainingApi.warmUpStream().first;
        final allNames = warmUpApi.map((warmUp) => warmUp.name).toList();
        if (widget.training != null) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(widget.training == null
            ? 'Nuevo Calentamiento'.toUpperCase()
            : 'Editar Calentamiento'.toUpperCase()),
        actions: <Widget>[
          FlatButton(
            child: Icon(
              Icons.save,
  ...
  uploadImageToFirebase() {
    final String path = 'warm_up/warm_up_${DateTime.now().toString()}.jpg';
