...
class EditRacePage extends StatefulWidget {
  EditRacePage({Key key, @required this.trainingApi, this.training})
      : super(key: key);
  ...
  @override
  _EditRacePageState createState() => _EditRacePageState();
}

class _EditRacePageState extends State<EditRacePage> {
  ...
  Future<void> _submit() async {
    if (_validateAndSaveForm()) {
      try {
        final brigades = await widget.trainingApi.raceStream().first;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 2.0,
        title:
            Text(widget.training == null ? 'Nuevo Carrera' : 'Editar Carrera'),
        actions: <Widget>[
          FlatButton(
            child: Icon(
              Icons.save,
  ...
  uploadImageToFirebase() {
    final String path = 'race/race_${DateTime.now().toString()}.jpg';
