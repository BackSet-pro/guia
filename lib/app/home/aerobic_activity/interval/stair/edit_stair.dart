...
class EditStairPage extends StatefulWidget {
  EditStairPage({Key key, @required this.trainingApi, this.training})
      : super(key: key);
  ...
  @override
  _EditStairPageState createState() => _EditStairPageState();
}

class _EditStairPageState extends State<EditStairPage> {
  ...
  void initState() {
    ...
    if (widget.training != null) {
      ...
      _pause = widget.training.pausa.toString();
  ...
  Future<void> _submit() async {
    if (_validateAndSaveForm()) {
      try {
        final brigades = await widget.trainingApi.stairStream().first;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 2.0,
        title: Text(
            widget.training == null ? 'Nueva Escalera' : 'Editar Escalera'),
        actions: <Widget>[
          FlatButton(
            child: Icon(
              Icons.save,
  ...
  uploadImageToFirebase() {
    final String path = 'stair/stair_${DateTime.now().toString()}.jpg';
